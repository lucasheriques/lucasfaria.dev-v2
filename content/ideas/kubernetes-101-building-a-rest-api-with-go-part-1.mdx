...
Pods are temporary. They can come and go. Maybe due to scaling, updates, or failures. No matter why, Kubernetes manages that life cycle for us. Ensuring they're rescheduled and maintained according to the desired state.
...
I built this Kubernetes Visualizer component to help visualize K8s self healing capabilities. It showcases how, even after a Pod is killed, K8s can create new ones. And we can see that the service name stays the same, and its age doesn't change, even when all pods are down.
