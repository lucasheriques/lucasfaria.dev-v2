...
And I have a growing interest in distributed systems. Understanding how companies like Meta and Google serve billions of users with an insanely low down time considering their scale.
...
It’s been a while since I did similar like this. Learn a new tool. Build a new application that solves some pain for me and for others, using a new architecture. And I'm curious as well if we can make a simple enough Kubernetes application that feels as much productive as serverless.
...
There's a few options here. It'll vary depending on what system are you on. I'm using a MacBook Pro, then consider this if you're following along. For Ubuntu users, [MicroK8S](https://microk8s.io/) appears to be a good choice. There's also [Rancher Desktop](https://rancherdesktop.io/), which is cross-platform and also has a local
...
With Kubernetes, services and apps are running inside pods, isolated from the host operational system. This is to ensure security and resource encapsulation. That means that, even if our app is running locally, we can't just `localhost` to it. We'd have to explicitly expose the Kubernetes network to ours.
...
What you need to understand about Kubernetes is that pods are temporary. They can come and go. Maybe due to scaling, updates, or failures. No matter why, Kubernetes manages that life cycle for us. Ensuring they're rescheduled and maintained according to the desired state.

Eventually, we can even introduce load balancers to manage traffic distribution across them.

Services are more permanent. They provide an abstraction layer to the set of pods. In our case, we have just one, as that's the replica number we used in the deployment YAML. With a service, Kubernetes assigns it a stable IP address and DNS name. This way, we can enable internal and external access to reach the pods. Even when they get created, destroyed or updated.
